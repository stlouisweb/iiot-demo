  ...
  ClientImpl.prototype.subscribe = function (filter, subscribeOptions) {
    ...
    if (subscribeOptions.onFailure) {
      wireMessage.onFailure = function (errorCode) {
        subscribeOptions.onFailure({invocationContext: subscribeOptions.invocationContext, errorCode});
      };
    }
...
=======
          ...
          if (messageBytes[i] <= 0xF)
				  ...
				  hex += messageBytes[i].toString(16);
>>>>>>> update-valve-message
			  }
  ...
  ClientImpl.prototype._disconnected = function (errorCode, errorText) {
    ...
    if (this.connectOptions.uris && this.hostIndex < this.connectOptions.uris.length - 1) {
      ...
      if (this.connected) {
        ...
        // Execute the connectionLostCallback if there is one, and we were connected.
        if (this.onConnectionLost)
          this.onConnectionLost({errorCode, errorMessage: errorText});
      } else {
        // Otherwise we never had a connection, so indicate that the connect has failed.
        if (this.connectOptions.mqttVersion === 4 && this.connectOptions.mqttVersionExplicit === false) {
          this._trace('Failed to connect V4, dropping back to V3');
...
<<<<<<< HEAD
	var Message = function (newPayload) {
		...
		this._getPayloadString = function () {
			...
			// The above logic will break the app because we are passing strings
			// with integers appended to them.
			// just return the raw payload instead:
			return payload;
		};
